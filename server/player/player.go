...
func (p *Player) Tick(w *world.World, current int64) {
	...
	if p.session() == session.Nop {
		if !p.Immobile() {
			m := p.mc.TickMovement(p, pos, p.Velocity(), cube.Rotation{p.yaw.Load(), p.pitch.Load()})
			m.Send()
		...
		yaw, pitch := p.Rotation().Yaw(), p.Rotation().Pitch()
		...
		if !deltaPos.ApproxEqualThreshold(mgl64.Vec3{}, 0.001) || math.Abs(lastYaw-yaw)+math.Abs(lastPitch-pitch) > 1.0 {
			...
			for _, v := range p.viewers() {
				v.ViewEntityMovement(p, pos, cube.Rotation{yaw, pitch}, p.OnGround())
			}
